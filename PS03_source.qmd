...
#option 1
tail(txhousing$city, n = 1);
#option 2
txhousing$city[nrow(txhousing)]
#option 3
txhousing |>
  slice_tail(n = 1) |>
  pull(city)
#option 4
txhousing |>
  summarize(last_city = last(city))
```

- The last city listed in the dataset is `r tail(txhousing$city, n = 1)`.
:::
...
```
- The following code does not give the requested answer because & takes precedence over or (like PEMDAS).
- `!` (NOT)
- `&` `&&` Logical AND
- `|`  `||` Logical OR
:::
...
# Type your code and comments inside the code chunk
txhousing |>
  filter(city == "Dallas", year == 2012 | year == 2013) -> dallas_sub
dim(dallas_sub) #verify the dimensions, here it says dallas_sub contains 24 rows and 8 columns
head(dallas_sub) #view the first six rows
...
tail(dallas_sub, 6) #views the last 6 rows of the dataset
```
:::
...
#na.rm = TRUE : if theres any missing (NA) datapoints in the dataset, if wont cause final calc to return NA.
dallas_summary #display data frame
```
:::
...
#you do not need to save results means dont need to save to anything, like with <-
```
:::
...
- The code calculated the average of the monthly sales percentage, it grouped the data by month, and it filtered the years 2012 and 2013.
:::
...
```{r}
# Type your code and comments inside the code chunk
fewest_listings <- txhousing |> 
  filter(year == 2015, month == 1) |>
  arrange(listings) |>
  slice(1) #selects a specific row from tibble by their position number

#inline code answer: - In January of 2015, `r fewest_listings$city` had the fewest houses listed for sale with a total of `r fewest_listings$listings` houses listed.
```

- In January of 2015, `r fewest_listings$city` had the fewest houses listed for sale with a total of `r fewest_listings$listings` houses listed.
:::
...
```{r}
# Type your code and comments inside the code chunk
sales_2012 <- txhousing |>
  filter(year == 2012) |>
  group_by(month) |>
  summarize(total_sales = sum(sales, na.rm = TRUE)) |>
              arrange(desc(total_sales))
#inline code answer: - The most houses were sold In Texas during month `r sales_2012$month[1]` (June), with a total of `r format(sales_2012$total_sales[1], big.mark = ",")` houses sold.
```

- The most houses were sold In Texas during month `r sales_2012$month[1]` (June), with a total of `r format(sales_2012$total_sales[1], big.mark = ",")` houses sold.
:::
